SELECT [1, 1, 1, 2];
-- To convert this to rows use UNNEST

-- Returns all but with duplicates removed
SELECT UNNEST ([1, 1, 1, 2])
UNION
SELECT UNNEST ([1, 1, 3]);

-- Returns all and with duplicates
SELECT UNNEST ([1, 1, 1, 2])
UNION ALL
SELECT UNNEST ([1, 1, 3]);

-- Returns the intersection but removes the duplicates
SELECT UNNEST ([1, 1, 1, 2])
INTERSECT
SELECT UNNEST ([1, 1, 3]);

-- Returns the intersection and keeps duplicates
SELECT UNNEST ([1, 1, 1, 2])
INTERSECT ALL
SELECT UNNEST ([1, 1, 3]);
...
-- Returns rows in A not in B. Duplicates removed one for one. Basically A - B
SELECT UNNEST ([1, 1, 1, 2])
EXCEPT ALL
SELECT UNNEST ([1, 1, 3]);
